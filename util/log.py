...
class init_log:
    ...
    def get_path(self):
        ...
        self.path = f"log/{new_filename}"

    def save_log(self, write_str):
        if path.exists(self.path) and path.getsize(self.path) > self.max_file_size:
            old_path = self.path
            self.get_path()
            with open(old_path, "a", encoding="utf-8") as file:
                file.write(f"{datetime.now().strftime('[%y/%m/%d %H:%M:%S]')}[INFO]\t日志文件大小超过上限, 后续日志分割至{self.path}\n")
